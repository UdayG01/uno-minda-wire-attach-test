...
# This contains the detect_hand class not in connector_detector2.py
...
class ConnectorDetector:
    ...
    def detect_hand(self, frame: np.ndarray) -> Dict[str, Any]:
        """
        Detect hand and calculate its centroid in frame
        
        Args:
            frame: Input frame (BGR format)
            
        Returns:
            Dictionary containing hand detection data with mask and metadata
        """
        if self.model is None:
            return {}
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                # Check if we have detections
                if result.boxes is not None and len(result.boxes) > 0:
                    # Get boxes and ensure they're on CPU
                    boxes = result.boxes.data.cpu().numpy()
                    ...
                    for i, box in enumerate(boxes):
                        ...
                        # Check if the detection is a hand
                        if self.class_names[class_id].lower() == 'hand':
                            hand = {
                                'bbox': [int(x1), int(y1), int(x2), int(y2)],
                                'confidence': float(conf),
                                'class_id': class_id,
                            ...
                            if masks is not None and i < len(masks):
                                ...
                                hand['mask'] = mask.astype(np.uint8)
                                
                                # Calculate centroid and area using moments
                                moments = cv2.moments(hand['mask'])
                                if moments['m00'] > 0:
                                    cx = int(moments['m10'] / moments['m00'])
                                    cy = int(moments['m01'] / moments['m00'])
                                    hand['centroid'] = (cx, cy)
                                    hand['area'] = moments['m00']
                                else:
                                    # Fallback to bbox center
                                    cx = int((x1 + x2) / 2)
                                    cy = int((y1 + y2) / 2)
                                    hand['centroid'] = (cx, cy)
                                    hand['area'] = (x2 - x1) * (y2 - y1)
                            else:
                                # No mask available, use bbox center
                                cx = int((x1 + x2) / 2)
                                cy = int((y1 + y2) / 2)
