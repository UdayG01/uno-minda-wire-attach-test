...
class RobustConnectorTracker:
    ...
    def __init__(self, max_disappeared: int = 30, 
        ...
        self.auto_reset_delay = 30.0  # 30 seconds delay
    ...
    def _analyze_push_interaction(self, track: TrackedConnector, terminals: List[Dict[str, Any]], hand_detection: Optional[Dict[str, Any]] = None):
        """
        Robust push analysis with proximity-based terminal assignment and hand movement detection
        Args:
            track: Connector being tracked
            terminals: List of terminals
            hand_detection: Optional hand detection data for movement-based validation
        """
        ...
        self._update_push_session(track, track.assigned_terminal, smoothed_depth, terminals, hand_detection)
    
    def _update_push_session(self, track: TrackedConnector, terminal_id: int, depth: float, terminals: List[Dict[str, Any]], hand_detection: Optional[Dict[str, Any]] = None):
        """
        Update push session for connector-terminal pair with hand movement detection
        Args:
            track: Current connector being tracked
            terminal_id: ID of the terminal being interacted with
            depth: Current depth of connector in terminal
            terminals: List of all terminals
            hand_detection: Optional hand detection data with centroid
        """
        ...
        # Check for hand movement after first push
        if terminal_state['push_count'] == 1 and hand_detection and 'centroid' in hand_detection:
            if 'initial_hand_y' not in terminal_state:
                terminal_state['initial_hand_y'] = hand_detection['centroid'][1]
            elif terminal_state['initial_hand_y'] is not None:
                current_hand_y = hand_detection['centroid'][1]
                y_movement = abs(current_hand_y - terminal_state['initial_hand_y'])
                
                if y_movement >= self.hand_movement_threshold:
                    print(f"🤚 Significant hand movement detected: {y_movement:.1f}px")
                    # Simulate second push completion
                    session['push_count'] = 2
                    self._end_push_attempt(track, terminal_id, session, terminals, True)
                    terminal_state.pop('initial_hand_y', None)  # Clean up after use
                    return
...
def test_robust_tracking_system(model_path: str, video_source: Optional[str] = None,
    ...
    print("🔧 Intel XPU acceleration enabled for Intel Arc Graphics")  
    ...
    tracker = RobustConnectorTracker(
        ...
        push_depth_threshold=1.7,       # 2px inside terminal boundary
        min_push_duration=1,             # ~0.27 seconds minimum push
        second_push_timeout=30,         # 5 seconds for second push
